// https://github.com/eWert-Online/warp/blob/master/src/Warp/Warp_XHR.re
...
//   t,
//   @string [@bs.as("arraybuffer") #arrayBuffer | #document | #json | #text],
// ) => string = "responseType"
...
external open_: (
  t,
  ~method: @string [@as("GET") #Get | @as("POST") #Post | @as("PUT") #Put | @as("DELETE") #Delete],
  ~url: string,
  unit,
) => unit = "open"

@send
external openWithAsync: (
  t,
  ~method: @string [@as("GET") #Get | @as("POST") #Post | @as("PUT") #Put | @as("DELETE") #Delete],
  ~url: string,
  ~async: bool,
...
@send external sendFormData: (t, Dom2.FormData.t) => unit = "send"

@send external sendArrayBuffer: (t, Js.Typed_array.array_buffer) => unit = "send"
...
// onError:
// It's important to note that this is only called if there's an error at the network level.
// If the error only exists at the application level (e.g. an HTTP error code is sent),
